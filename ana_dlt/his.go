...
func CalAllComb() (combs []string) {
	...
	for i := 0; i <= 5; i++ {
		for j := 0; j <= 2; j++ {
			...
			combs = append(combs, gen.CrossComb2(afs, abs, i, j)...)
		}
	}
	return
}
...
func WriteFBCombToFile() {
	...
	for i := 0; i <= 5; i++ {
		for j := 0; j <= 2; j++ {
			...
			combs = append(combs, gen.CrossComb2(afs, abs, i, j)...)
			if i == 0 && j == 1 {
				f0b1 = append(f0b1, gen.CrossComb2(afs, abs, i, j)...)
			}
			if i == 0 && j == 2 {
				f0b2 = append(f0b2, gen.CrossComb2(afs, abs, i, j)...)
			}

			if i == 1 && j == 0 {
				f1b0 = append(f1b0, gen.CrossComb2(afs, abs, i, j)...)
			}

			if i == 1 && j == 1 {
				f1b1 = append(f1b1, gen.CrossComb2(afs, abs, i, j)...)
			}

			if i == 1 && j == 2 {
				f1b2 = append(f1b2, gen.CrossComb2(afs, abs, i, j)...)
			}

			if i == 2 && j == 0 {
				f2b0 = append(f2b0, gen.CrossComb2(afs, abs, i, j)...)
			}
			if i == 2 && j == 1 {
				f2b1 = append(f2b1, gen.CrossComb2(afs, abs, i, j)...)
			}

			if i == 2 && j == 2 {
				f2b2 = append(f2b2, gen.CrossComb2(afs, abs, i, j)...)
			}

			if i == 3 && j == 0 {
				f3b0 = append(f3b0, gen.CrossComb2(afs, abs, i, j)...)
			}
			if i == 3 && j == 1 {
				f3b1 = append(f3b1, gen.CrossComb2(afs, abs, i, j)...)
			}
			if i == 3 && j == 2 {
				f3b2 = append(f3b2, gen.CrossComb2(afs, abs, i, j)...)
			}
		}
	}
	...
	for _, dlt := range dlts {
		...
		for i := 0; i <= 5; i++ {
			for j := 0; j <= 2; j++ {
				...
				itemCombs = append(itemCombs, gen.CrossComb2(fs, bs, i, j)...)
			}
		}
		if tempNum == 0 {
			tempNum++
		...
		week, _ := gen.GetWeekday(dlt.DrawTime)
...
func MinComb(dlts []models.Dlt, eqN int, accumulateStartDrawNum string) (drawNum2Type2NotExistCount map[string]map[string]*DrawNumNotExistComb) {
	...
	for _, dlt := range dlts {
		...
		for _, fbCount := range fbCounts {
			itemCombs := gen.CrossComb2(fs, bs, fbCount.FCount, fbCount.BCount)
...
func CalUniqueTyp() {
	typs, _ := dbop.ReadAllTyp(false)
	uniqueDltIds := make([]uint, 0)
	for _, typ := range typs {
		if !slices.Contains(uniqueDltIds, typ.DltID) {
			uniqueDltIds = append(uniqueDltIds, typ.DltID)
		}
	}
	fmt.Println(len(uniqueDltIds))
...
func InitDlts() {
	if cfg.Default.EnvIsLocal == 2 {
		ZxDlts, _ = dbop.ReadAllDlt(false)
		DxDlts, _ = dbop.ReadAllDlt(true)
	} else {
		...
		for i := 0; i < 2; i++ {
			...
			//		defer wg.Done()
			//		startTime := time.Now()
			//		ZxAllDlts, _ = dbop.ReadAllDlts(false)
			//		fmt.Println(time.Now().Sub(startTime).Round(time.Second), time.Now().Sub(startTime0).Round(time.Second))
			//	}()
			//}
		}
