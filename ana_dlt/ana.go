...
func FMaxN(dlts []models.Dlt, fN int) (counts []*NumCount) {
	...
	for _, dlt := range dlts {
		...
		nums := gen.Comb([]string{dlt.F1, dlt.F2, dlt.F3, dlt.F4, dlt.F5}, fN)
...
func FBMaxN2M(dlts []models.Dlt, fN, bM int) (counts []*NumCount) {
	...
	for _, dlt := range dlts {
		...
		week, _ := gen.GetWeekday(dlt.DrawTime)
		nums := gen.CrossComb([]string{dlt.F1, dlt.F2, dlt.F3, dlt.F4, dlt.F5}, []string{dlt.B1, dlt.B2}, fN, bM)
		for _, num := range nums {
			_, ok = num2Count[num]
			if ok {
				num2Count[num].Count = num2Count[num].Count + 1
...
func BMaxN(dlts []models.Dlt, bN int) (counts []*NumCount) {
	...
	for _, dlt := range dlts {
		...
		week, _ := gen.GetWeekday(dlt.DrawTime)
		nums := gen.Comb([]string{dlt.B1, dlt.B2}, bN)
		for _, num := range nums {
			_, ok = num2Count[num]
			if ok {
				num2Count[num].Count = num2Count[num].Count + 1
