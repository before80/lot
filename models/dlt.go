...
type Dlt struct {
	...
	DataSrc          int       `gorm:"default:0;column:data_src" json:"data_src"`
	CreatedAt        time.Time `json:"created_at"`
}

type Typ struct {
	ID        uint      `gorm:"primaryKey" json:"id"`
	DltID     uint      `json:"dlt_id"`
	PrevDltId uint      `json:"prev_dlt_id"`
	Typ_1     uint8     `json:"typ_1"`
	Typ_2     uint8     `json:"typ_2"`
	Hm        string    `json:"hm"`
	CreatedAt time.Time `json:"created_at"`
}
